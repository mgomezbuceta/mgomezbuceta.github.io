    ...
    $("#contact-form").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            ...
            grecaptcha.ready(function() {
                grecaptcha.execute('6LcrTJ0aAAAAAOPHTepGCTq1jbHixLLDWDgiDcVq', {
                    action: 'send_form_contact'
                }).then(function(token) {
                    $('#contact-form').prepend('<input type="hidden" id="recaptcha" name="g-recaptcha-response" value="' + token + '">');
                    submitForm();
                    /*$.post("assets/php/form-contact.php",{name: name, email: email, message: message, token: token}, function(result) {
                            console.log(result);
                            if(result.success) {
